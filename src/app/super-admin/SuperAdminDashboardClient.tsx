...
export function SuperAdminDashboardClient({
  ...
  const handleToggleMember = async (outletId: string, outletName: string, currentStatus: boolean) => {
    const newStatus = !currentStatus;
    try {
      const res = await toggleOutletMembershipAction(outletId, newStatus);
      if (res.success) {
        showSuccessAlert(newStatus ? "Member Diaktifkan" : "Member Dinonaktifkan", res.message);
        setLocalOutlets((prev) =>
          prev.map((o) => (o.id === outletId ? { ...o, isMember: newStatus } : o))
        );
        router.refresh();
      } else {
      ...
      showErrorAlert("Error", "Gagal mengubah status member.");
    }
  };
...
Tim Layanan Smart QR`;
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-slate-100 flex relative selection:bg-indigo-500 selection:text-white">
                ...
                <table className="w-full text-left text-xs text-slate-300">
                  <thead className="bg-slate-950/60 text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-800">
                    <tr>
                      ...
                      <th className="py-3 px-4">Kode Kartu</th>
                      <th className="py-3 px-4 text-center">Total Scan</th>
                      <th className="py-3 px-4 text-center">Status Member</th>
                      <th className="py-3 px-4 text-right">Aksi</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/60">
      ...
      {/* Modal Kelola Bukti Transfer & Harga Member (Super Admin) */}
